...
const AddClass = ({params}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:4000/classes/${params.class}`, {
        method: "PATCH",
        headers: {
      ...
      await response.json();
      localStorage.setItem("toast",JSON.stringify({type: "success", message: "Class has been updated successfully!"}))
      // setTimeout(() => {
